    ...
    <div class="content">
      <van-col span="24">
        <van-tabs @click="onClick" sticky>
          <van-tab :title="item.name" v-for="item in category" :key="item.id" :name="item.id">
            <van-list
              v-model="loading"
              :finished="finished"
              finished-text="没有更多了"
...
export default {
  ...
  computed: {
    ...mapState("supermarket", ["category", "supershop"]),
    ...mapState("user", ["userinfo"])
  },
  created() {
    this.findAllCategory().then(res => {
      this.findSuperByCaId(parseInt(res.data[0].id)).then(() => {
        this.finished = true;
        this.loading = false;
      });
    });
  ...
  methods: {
    ...
    onClick(name, title) {
      console.log("商品类别编号：", parseInt(name));
      // 根据商品类别编号查询商品列表
      this.findSuperByCaId(parseInt(name)).then(() => {
        this.finished = true;
        this.loading = false;
      });
    },
